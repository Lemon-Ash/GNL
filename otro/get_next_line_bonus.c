...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/08/10 06:24:29 by rnavarre          #+#    #+#             */
/*   Updated: 2020/08/10 12:28:50 by rnavarre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	buffer[258][BUFFER_SIZE + 1];
	int			status;
	char		*prev;
	char		*out;
	...
	{
		if (!((out) = readbuffer(&status, buffer[fd], '\n')))
			break ;
		...
		if (status)
			return (1);
		if ((status = read(fd, buffer[fd], BUFFER_SIZE)) != -1)
			buffer[fd][status] = '\0';
		if (!status)
			return (0);
		prev = *line;
